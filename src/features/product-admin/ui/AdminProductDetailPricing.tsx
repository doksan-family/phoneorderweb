...
type Preview = ReturnType<typeof usePricingPreview>;
type VariantRow = { id: string; storageValue: string; releasePrice: number };
...
function PlanPricingRow({
  ...
  const subTypes: string[] = entry.subscriptionTypes.filter(
    ...
    <section className="grid gap-4 rounded-xl border border-slate-200 p-4">
      <div>
        ...
        <p className="mb-0 mt-1 text-xs leading-relaxed text-slate-500">
          저장된 조건과 현재 가격 정책으로 계산한 예상 금액입니다. 요금제·가입유형별로
          공시지원금·선택약정 결과를 보여줍니다.
        </p>
      </div>
